"""AnnData builder for CRISPRSCope h5ad export."""
...
class CRISPRSCopeAnnDataBuilder:
    """Assemble an AnnData object from CRISPRSCope pipeline outputs."""
    ...
    def __init__(
        ...
        allele_parquet_paths: List[Path],
    ) -> None:
        self.config = config
        ...
        self.cell_barcodes = self.editing_summary.index.astype(str).tolist()
        ...
        logger.info("Configured AnnData builder for %d cells and %d amplicons.", self.n_obs, self.n_vars)
    ...
    def _build_var(self) -> pd.DataFrame:
        ...
        if valid_count_cols:
            counts_matrix = self.editing_summary[valid_count_cols].copy()
        ...
        if valid_mod_cols:
            mod_matrix = self.editing_summary[valid_mod_cols].copy()
    ...
    def _build_allele_layers_iteratively(self) -> Dict[str, np.ndarray]:
        ...
        for path in self.allele_parquet_paths:
            ...
            df["total_reads"] = df.groupby(
                ["cell_barcode", "amplicon_name"],
                observed=True,
            )["count"].transform("sum")
        ...
        for key in ["allele_seq_1", "allele_seq_2"]:
            allele_layers[key] = np.char.encode(
                allele_layers[key].astype(str),
                "utf-8",
            ).astype(f"S{max_len_seen}")
    ...
    def build(self) -> AnnData:
        ...
        cond_hom = (
            (mod_pct_matrix >= zyg_params["hom_min_mod_pct"]) &
        ...
        adata.uns["CRISPRSCope_h5ad_config"] = self.config
        adata.uns["scCRISPR_settings"] = self.settings
        adata.uns["scCRISPR_h5ad_config"] = self.config
        adata.uns["zygosity_encoding"] = {
            "0": "WT/WT",
            "1": "WT/Mut",
            "2": "Mut/Mut (Homozygous)",
            "3": "Mut/Mut2 (Compound Het)",
            "-1": "NoData",
        }
        return adata


ScCRISPRAnnDataBuilder = CRISPRSCopeAnnDataBuilder
